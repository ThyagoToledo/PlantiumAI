"""
PlantiuIA — BlackBox AI Provider
API OpenAI-compatível via BlackBox (https://api.blackbox.ai)
"""
...
class BlackboxProvider(BaseProvider):
    """Provedor de IA via BlackBox AI (API OpenAI-compatível)."""

    name = "blackbox"
    BASE_URL = "https://api.blackbox.ai/v1"

    def __init__(self, api_key: str, model: str = "blackboxai/anthropic/claude-sonnet-4.6"):
    ...
    async def analyze_image(
        ...
        """Analisa imagem via BlackBox AI (vision)."""
        ...
        try:
            ...
            logger.info(f"BlackBox [{self.model}]: Analise concluida em {latency:.0f}ms ({tokens} tokens)")
            ...
            latency = (time.time() - start_time) * 1000
            error_msg = f"Erro BlackBox: {str(e)}"
    ...
    async def chat(
        ...
        try:
            ...
            logger.info(f"BlackBox [{self.model}]: Chat concluido em {latency:.0f}ms ({tokens} tokens)")
